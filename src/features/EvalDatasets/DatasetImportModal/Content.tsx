...
import { getPresetById } from '../datasetPresets';
...
const DatasetImportContent: FC<DatasetImportContentProps> = ({
  ...
  useEffect(() => {
    if (!initialImportState) return;

    setStep(1);
    setPathname(initialImportState.pathname);
    setFilename(initialImportState.filename || '');
    setHeaders(initialImportState.headers);
    setPreview(initialImportState.preview);
    setTotalCount(initialImportState.totalCount);
    setFormat(initialImportState.format);
    setMapping(initialImportState.mapping || autoInferMapping(initialImportState.headers, preset));
    setDelimiter('');
  }, [initialImportState, preset]);
