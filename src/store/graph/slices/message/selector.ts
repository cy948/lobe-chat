...
export const getNodeMessages =
  (s: GraphStore) =>
  (nodeId: string): ChatMessage[] | undefined => {
...
export const getActiveNodeMessageById =
  (s: GraphStore) =>
...
export const isMessageLoading = (s: GraphStore) => (msgId: string) =>
  s.messageLoadingIds.includes(msgId);

export const isMessageEditing = (s: GraphStore) => (msgId: string) =>
  s.messageEditingIds.includes(msgId);
